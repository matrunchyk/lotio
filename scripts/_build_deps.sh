...
################################################################################
# Shared Dependencies Build Script
#
# This script builds dependencies (Skia and nlohmann/json) for either native (binary) or WASM targets.
# It handles Skia setup, dependency fetching, and compilation.
#
# Usage:
#   scripts/_build_deps.sh [--target=binary|wasm] [--target-cpu=arm64|x64] [--skip-setup]
#
# Parameters:
...
# Set THIRD_PARTY_DIR early so it's available everywhere
...
if [[ "$SKIP_SETUP" == "false" ]]; then
    ...
    # Remove existing third_party if it exists (but preserve nlohmann)
    NLOHMANN_TEMP=""
    if [ -d "$THIRD_PARTY_DIR" ]; then
        echo "   Removing existing third_party directory (preserving nlohmann)..."
        # Preserve nlohmann directory if it exists
        if [ -d "$THIRD_PARTY_DIR/nlohmann" ]; then
            NLOHMANN_TEMP=$(mktemp -d)
            mv "$THIRD_PARTY_DIR/nlohmann" "$NLOHMANN_TEMP/nlohmann"
        fi
...
EOF
    ...
    # Restore nlohmann if it was preserved, or download it if it doesn't exist
    if [ -n "$NLOHMANN_TEMP" ] && [ -d "$NLOHMANN_TEMP/nlohmann" ]; then
        echo "   Restoring preserved nlohmann directory..."
        mkdir -p "$THIRD_PARTY_DIR"
        mv "$NLOHMANN_TEMP/nlohmann" "$THIRD_PARTY_DIR/nlohmann"
        rm -rf "$NLOHMANN_TEMP"
        echo "   ✅ nlohmann restored"
    elif [ ! -d "$THIRD_PARTY_DIR/nlohmann" ]; then
        echo "   Downloading nlohmann/json header..."
        mkdir -p "$THIRD_PARTY_DIR/nlohmann"
        curl -L -s -o "$THIRD_PARTY_DIR/nlohmann/json.hpp" https://github.com/nlohmann/json/releases/download/v3.12.0/json.hpp
        if [ -f "$THIRD_PARTY_DIR/nlohmann/json.hpp" ]; then
            echo "   ✅ nlohmann/json downloaded"
        else
            echo "   ⚠️  Warning: Failed to download nlohmann/json"
        fi
    fi
    echo ""
fi

# Ensure nlohmann exists even if Skia setup was skipped
if [ ! -d "$THIRD_PARTY_DIR/nlohmann" ] || [ ! -f "$THIRD_PARTY_DIR/nlohmann/json.hpp" ]; then
    echo "   Ensuring nlohmann/json is available..."
    mkdir -p "$THIRD_PARTY_DIR/nlohmann"
    curl -L -s -o "$THIRD_PARTY_DIR/nlohmann/json.hpp" https://github.com/nlohmann/json/releases/download/v3.12.0/json.hpp
    if [ -f "$THIRD_PARTY_DIR/nlohmann/json.hpp" ]; then
        echo "   ✅ nlohmann/json available"
    else
        echo "   ⚠️  Warning: Failed to download nlohmann/json"
    fi
fi
